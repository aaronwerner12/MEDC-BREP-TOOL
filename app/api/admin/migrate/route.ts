...
// One-time (idempotent) setup endpoint: creates the schema and seeds the
// watchlist, using the app's own DATABASE_URL connection. This exists so the
// database can be initialized without direct SQL-console access.
//
// Auth: requires CRON_SECRET, sent either as `Authorization: Bearer <secret>`
// (curl) or as a `?key=<secret>` query param (browser). Everything here is
// idempotent — create-if-not-exists and on-conflict-do-nothing — so it is safe

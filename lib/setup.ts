...
export async function ensureSchema(): Promise<SetupStep[]> {
  ...
  // Tracked as notable employers (segment defaults to 'medc') so the desk and
  // report watch them as expansion stories. Band is left null (headcount not
  // known); sector is set only where the name makes it unambiguous. The free
  // profile chain fills the rest. Idempotent.
  await run("seed Inc. 5000 2026 McKinney honorees", () => sql`
